...
const serviceAccount = require('./firebase-admin-config.json'); // Path to your downloaded JSON key
const readline = require('readline'); // Node.js built-in module for reading input
const util = require('util'); // For deep logging of objects
...
const appId = serviceAccount.project_id; // Use your project ID as the appId for consistency
...
async function generateChallengesWithGemini(numChallenges, topics, difficulties) {
    ...
    try {
        ...
        if (result.candidates?.[0]?.content?.parts?.[0]?.text) {
            ...
            // Clean the response: remove any leading/trailing markdown code blocks if present
            jsonText = jsonText.replace(/^```json\n|```$/g, '').trim();

            // --- NEW: Replace invalid JSON literals (NaN, undefined) with null ---
            ...
            return parsedChallenges.map(c => ({
                ...
                // Basic validation for critical fields before returning
                ...
                for (const tc of c.testCases) {
                    ...
                    // Check for undefined in expectedOutput, which is a common cause of Firestore errors
                    // Also check if expectedOutput is 'null' string if LLM accidentally outputs it.
        ...
        throw error; // Re-throw to be caught by the main seedChallenges catch block
    }
}
...
async function seedChallenges() {
    ...
    try {
        ...
        for (const challenge of challenges) {
            ...
            // --- NEW: Serialize input and expectedOutput to JSON strings before saving ---
            const challengeToSave = { ...challenge }; // Create a copy to modify
            challengeToSave.testCases = challenge.testCases.map(tc => ({
                input: JSON.stringify(tc.input), // Convert input array to string
                expectedOutput: JSON.stringify(tc.expectedOutput) // Convert expectedOutput to string
            }));
            // --- END NEW ---

            const docRef = collectionRef.doc(challenge.id);
            currentBatch.set(docRef, challengeToSave); // Save the modified challenge
            count++;
