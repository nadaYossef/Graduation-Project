...
import {
    getAuth,
    signInAnonymously,
    signInWithCustomToken, // Kept for potential Canvas environment, though not directly used in new UI
    onAuthStateChanged,
    createUserWithEmailAndPassword,
    signInWithEmailAndPassword,
    signOut,
    sendEmailVerification,
    sendPasswordResetEmail, // New: For forgot password functionality
    deleteUser,
    updateProfile
} from 'firebase/auth';
import {
    ...
    arrayUnion,
    arrayRemove,
    getDoc // Added to fetch current profile for update consistency
...
const firebaseConfig = {
    apiKey: "AIzaSyCwjM3VrTXmvbzjmANYTU87_smv7TOgqz4", // <--- IMPORTANT: Replace with your actual Firebase API Key
    authDomain: "datacraft-app.firebaseapp.com", // <--- IMPORTANT: Replace with your actual Firebase Project ID
    projectId: "datacraft-app", // <--- IMPORTANT: Replace with your actual Firebase Project ID
    storageBucket: "datacraft-app.firebasestorage.app", // <--- IMPORTANT: Replace with your actual Firebase Project ID
    messagingSenderId: "491174733279",
    appId: "1:491174733279:web:e76e47cdf559452df6506a",
    measurementId: "G-E3HCXYQ0FV"
};

// Use your Firebase projectId as the __app_id for consistency in Firestore paths
const __app_id = firebaseConfig.projectId;

// Initialize Firebase services once globally
const app = initializeApp(firebaseConfig);
const auth = getAuth(app);
const db = getFirestore(app);
const storage = getStorage(app);
const analytics = getAnalytics(app); // Analytics initialized as requested

// --- ZindaFloatingChat Component ---
// This component manages the Zinda AI chatbot's floating UI and interaction.
const ZindaFloatingChat = ({ chatMessages, setChatMessages, chatInput, setChatInput, handleChatInputChange, handleSendMessage, chatMessagesEndRef, selectedChallenge, currentPage }) => {
    ...
    // Auto-scroll messages to the bottom whenever messages update or chat window opens/closes
    useEffect(() => {
...
const App = () => {
    ...
    const [showLoginModal, setShowLoginModal] = useState(true); // Control visibility of login/register forms
    const [isRegisterMode, setIsRegisterMode] = useState(false); // Toggle between login and register
    const [email, setEmail] = useState('');
    const [password, setPassword] = useState('');
    const [mobileNumber, setMobileNumber] = useState(''); // State for optional mobile number
    const [authError, setAuthError] = useState(''); // Error messages for auth operations
    const [isAuthReady, setIsAuthReady] = useState(false); // Tracks if Firebase Auth state has been checked

    // App Navigation States
    const [currentPage, setCurrentPage] = useState('challenges'); // 'challenges', 'leaderboard', 'profile', 'zinda', 'learning'
    const [selectedChallenge, setSelectedChallenge] = useState(null); // Currently selected challenge for detail view

    // Challenges Data States
    const [challenges, setChallenges] = useState([]); // List of all coding challenges
    const [loadingChallenges, setLoadingChallenges] = useState(true); // Loading state for challenges

    // Code Editor & Test Result States
    const [userCode, setUserCode] = useState(''); // Code entered by the user
    const [testResults, setTestResults] = useState([]); // Results of running test cases

    // Zinda Chatbot States
    const [chatMessages, setChatMessages] = useState([]); // Messages in the chatbot UI
    const [chatInput, setChatInput] = useState(''); // Input field for chatbot
    ...
    const [filterTopic, setFilterTopic] = useState('All'); // 'All' or specific topic
    const [searchTerm, setSearchTerm] = useState(''); // Search term for challenges


    // --- Firebase Auth & User Profile Management ---
    // This useEffect hook listens for changes in the user's authentication state.
    // It's crucial for managing login/logout and fetching/creating user profiles.
    useEffect(() => {
        const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
            setCurrentUser(user); // Update the currentUser state

            if (user) {
                // If a user is signed in, hide the login modal
                setShowLoginModal(false);

                // Define the Firestore document reference for the user's profile
                // Path: `users/{__app_id}/userProfiles/{user.uid}` (Ensure this matches your Firestore rules!)
                ...
                const unsubscribeProfile = onSnapshot(userDocRef, (docSnap) => {
                    if (docSnap.exists()) {
                        // If the profile exists, set it in state
                        setUserProfile(docSnap.data());
                    } else {
                        // If the profile doesn't exist (e.g., new user), create a default one
                        ...
                        setDoc(userDocRef, {
                            ...
                            mobileNumber: null, // Default null for new users
                            photoURL: user.photoURL || null, // Default null for new users
                            displayName: user.displayName || user.email?.split('@')[0] || `User_${user.uid.substring(0, 6)}`
                        }).then(() => {
                            // After creation, set the newly created profile in state
                            setUserProfile({
                                points: 0,
                                level: 1,
                                badges: [],
                ...
                // Clean up the profile listener when the component unmounts or auth state changes
                return () => unsubscribeProfile();
            } else {
                // If no user is signed in, show the login modal and clear profile data
                setShowLoginModal(true);
                setUserProfile(null);
                setAuthError(''); // Clear errors when user signs out
            }
            setIsAuthReady(true); // Mark authentication state as ready after the initial check
        });
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            // onAuthStateChanged listener will handle updating states and hiding modal
            setEmail('');
            setPassword('');
        } catch (error) {
...
// ... (previous imports and state)
    ...
    const handleRegister = async () => {
        if (!registerEmail || !registerPassword || registerPassword.length < 6) {
        ...
        try {
            // 1. Create the user with email and password in Firebase Authentication
            const userCredential = await createUserWithEmailAndPassword(auth, registerEmail, registerPassword);
            const user = userCredential.user;

            // Ensure displayName is set from the registration form
            const displayName = registerDisplayName || registerEmail.split('@')[0];

            // 2. Create the user's profile document in Firestore
            // This ensures the profile data exists before sending verification email
            const userDocRef = doc(db, `users/${firebaseConfig.projectId}/userProfiles`, user.uid);
            await setDoc(userDocRef, {
                email: user.email,
                displayName: displayName, // Use displayName from registration or derived
                photoURL: user.photoURL || '', // Use photoURL if available, else empty string
                points: 0,
                level: 1,
                badges: [],
                completedChallenges: [],
            ...
            // Now the user's profile exists in Firestore
            await sendEmailVerification(user);
            alert('Registration successful! Please check your email to verify your account.');

            // 4. Update the user's profile with display name and photoURL if needed
            // This update is separate and can happen before or after email verification,
            // but often includes the display name provided during registration.
            await updateProfile(user, {
                displayName: displayName,
                photoURL: '' // Set initial photoURL if not provided, or link to default image
            });
            ...
            setCurrentPage('challenges'); // Redirect to challenges page after successful registration
            // No explicit sign-in here, as onAuthStateChanged will handle it after registration
            // and sendEmailVerification doesn't automatically sign out/in.
...
// ... (rest of the code)
    ...
    const handleAnonymousSignIn = async () => {
        ...
        try {
            ...
            // onAuthStateChanged will handle updating states and hiding modal
    ...
    const handleDeleteAccount = async () => {
        ...
        if (confirmDelete) {
            try {
                ...
                // Alternatively, use deleteDoc(userDocRef) if you want to completely remove the document.
                // Firebase Security Rules for `deleteDoc` might need to be `allow delete: if request.auth.uid == userId;`
                ...
                // Firebase often requires recent authentication for security-sensitive operations like deleteUser.
            }
        }
    };
    ...
    const handleProfilePictureUpload = async (event) => {
        ...
        try {
            ...
            // Update user's profile in Firebase Authentication (display name, photo URL)
    ...
    // Fetches challenges from Firestore once authentication state is ready
    useEffect(() => {
        if (!db || !isAuthReady) {
            return; // Only fetch challenges once Firebase is initialized and auth state is known
        }

        const fetchChallenges = async () => {
            setLoadingChallenges(true);
            try {
                // Ensure this path matches where your seed.js script uploads challenges
                ...
                const fetched = querySnapshot.docs.map(doc => {
                    const data = doc.data();
                    // Deserialize testCases input/output from JSON strings back to objects/arrays
                    const deserializedTestCases = data.testCases ? data.testCases.map(tc => {
                        try {
                            ...
                            return tc; // Return as is if parsing fails
                        }
                    }) : [];
                    ...
                    return {
                        ...
                        // Ensure starterCode exists, default to empty string if not provided
                        starterCode: data.starterCode || `function ${data.entryPoint || 'myFunction'}(/* args */) {\n  // Write your code here\n}\n`
                    };
                });
                 // Sort challenges by difficulty: Easy, Medium, Hard (then by title if same difficulty)
                fetched.sort((a, b) => {
                ...
                if (fetched.length > 0) {
                    // Automatically select the first challenge if challenges exist
        ...
        fetchChallenges();
    }, [db, isAuthReady]); // Dependencies: Firestore instance and Auth readiness


    const handleChallengeSelect = (challenge) => {
        setCurrentPage('challenges'); // Ensure we are on challenges page, then select detail
        setSelectedChallenge(challenge);
        setUserCode(challenge.starterCode || ''); // Set starter code from challenge
        setTestResults([]); // Clear previous test results
        setChatMessages([]); // Clear chat messages for new challenge context
    };
    ...
    // This is a client-side execution. For a real production app, this MUST be moved to a secure backend.
    const handleRunTests = useCallback(() => {
        ...
        try {
            // Safely create and get a reference to the user's function
            // This approach attempts to define the function and then return a reference to it.
            ...
            selectedChallenge.testCases.forEach((testCase, index) => {
                ...
                try {
                    // Inputs might be an array, or a single value.
                    // If testCase.input is `[1, 2]`, it means the function takes two args.
                    // If testCase.input is `[ [1,2,3], 4]`, it means the function takes two args, first being an array.
    ...
    }, [selectedChallenge, userCode]); // Recalculate if challenge or userCode changes
...
// ... (previous code)
    ...
    // Directly call handleRunTests and capture its output, or ensure it's synchronous for immediate use
    // For simplicity, let's assume handleRunTests updates a dedicated state
    // and we need to check that state immediately.
    // A better approach would be to refactor handleRunTests to return the results
    // instead of setting state, then set state and use the returned value.

    // For now, let's re-evaluate tests right before checking.
    // A more robust solution for production would involve running tests on a secure backend.
        const newTestResults = [];
        try {
            const userFunctionWrapperCode = `
                ${userCode};
        ...
        `   ;
            const getUserFunction = new Function(userFunctionWrapperCode);
            const userSolutionFunction = getUserFunction();
            ...
            selectedChallenge.testCases.forEach((testCase, index) => {
                ...
                try {
                    const inputs = Array.isArray(testCase.input) ? testCase.input : [testCase.input];
                    actualOutput = userSolutionFunction(...inputs);
            ...
            setTestResults(newTestResults); // Update the state immediately after computation

        } catch (e) {
            console.error("Error parsing or getting user function:", e);
            setTestResults([{ type: 'error', message: `Code Setup Error: ${e.message}. Ensure your function "${selectedChallenge.entryPoint}" is correctly defined.`, passed: false }]);
            alert("Code Setup Error. Please check your function definition.");
            return; // Exit if there's a code setup error
        }

    // Now, check the results directly from newTestResults
        const allTestsPassed = newTestResults.length > 0 && newTestResults.every(result => result.passed);
        ...
        if (allTestsPassed) {
            ...
            if (alreadyCompleted) {
                pointsEarned = 0; // No points if already completed
                alert("You've already completed this challenge!");
            } else {
                let newBadge = `${selectedChallenge.title} Master`; // Award a badge for completing the challenge
                ...
                setAuthError(''); // Clear any previous auth errors
                setCurrentPage('challenges'); // Go back to challenges list
            }
        } else {
    ...
    // --- User Progress and Badge Management ---
    const updateUserProgress = async (userId, pointsToAdd, completedChallengeId = null, specificBadge = null) => {
        ...
        try {
            // Get the current profile data to ensure updates are based on the latest state
            ...
            // General Badge rules (can be expanded)
            ...
            await updateDoc(userDocRef, {
                ...
                level: updatedLevel,
                badges: Array.from(updatedBadges), // Convert Set back to Array for Firestore
                completedChallenges: Array.from(updatedCompletedChallenges)
            });

            // Update leaderboard entry (public data)
    ...
    useEffect(() => {
        if (!db || !isAuthReady) return; // Only fetch if Firebase is ready
    ...
    const handleSendMessage = async () => {
        ...
        try {
            // Construct context for Zinda, providing relevant details based on current page/challenge
            ...
            let chatHistory = [];
            chatHistory.push({ role: "user", parts: [{ text: fullPrompt }] }); // Use the fullPrompt here
            const payload = { contents: chatHistory };

            // IMPORTANT: Never expose your API key directly in client-side code in a production environment.
            // Use a Firebase Cloud Function or similar backend to proxy requests to Gemini.
            const geminiApiKey = "AIzaSyBPNhcZlMH06cR17jBaDl-MVqTrGOmIIqg"; // Your provided Gemini API Key
    ...
    // This function decides which page content to render based on the 'currentPage' state.
    const renderMainContent = () => {
        if (!isAuthReady) {
            // Show a loading screen or spinner while auth state is being determined
            return (
